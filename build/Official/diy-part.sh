...
export Op_name="NeoBird"                # 修改主机名称为OpenWrt(填0为不作修改)
...
export Customized_Information="NeoBird Offical $(TZ=UTC-8 date +%Y-%m%d-%H%M)"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
# sed -i '16s/"iStore"/"应用"/g' `egrep "iStore" -rl feeds/langge1/luci-app-store/luasrc/controller/store.lua`
